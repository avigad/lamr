...
def parseEdgeFile (lines : Array String) : Except String Graph := do
  ...
  let ["p", "edge", nVerts, nEdges] ← header |>.splitOnNonEmpty " "
    | throw s!"Invalid EDGE header: {header}"
  let (some nVerts, some nEdges) ← (nVerts.toNat?, nEdges.toNat?)
  ...
  let mut edges := #[]
  for ln in lines do
    let ["e", u, v] ← ln.splitOnNonEmpty " "
      | throw s!"Invalid line: {ln}"
    let (some u, some v) ← (u.toNat, v.toNat?)
